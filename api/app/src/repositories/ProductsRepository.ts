...
export class ProductsRepository {
    async create({ name, description, category, price, avatar, ingredients }: ProductData) {
        const [result] = await db('products').insert({ name, description, category, price, avatar }).returning('id')
        ingredients.forEach(async (ingredient) => {
            await db('ingredients').insert({ name: ingredient.name, product_id: result.id })
        })
        return result.id
    }
    async findProductsByNameOrIngredients(input: string) {
        if (input) {
            const ingredients = input.split(' ').map(ingredient => ingredient.replace(',', '').trim())
            if (ingredients.length < 1) {
                ...
                const productsWithIngredients = products.map((product) => {
                    const productsIngredients = allIngredients.filter(ingredient => ingredient.product_id === product.id)
