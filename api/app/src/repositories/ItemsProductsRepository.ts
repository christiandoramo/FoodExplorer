...
export class ItemsProductsRepository {
    async create({ product_id, user_id, quantity }: { product_id: string, user_id: string, quantity: number }) {
        ...
        const [result] = await db('items_products').insert({ product_id, cart_id: cart.id, amount, quantity }).returning('id')
    }
