...
export class FavoritesController {
    async create(request: ExtendedRequest, response: Response) {
        ...
        const newFavorite = await favoritesCreateService.execute({ userId: user.id, productId })
        return response.status(201).json(newFavorite)
    }
    async index(request: ExtendedRequest, response: Response) {
        const { user } = request
        if (!user)
            return response.status(401).json({ error: 'Unauthorized' }); // unauthorized de nao autenticado diferente de 403
        const favoritesRepository = new FavoritesRepository()
        const favoritesSearchService = new FavoritesSearchService(favoritesRepository);
