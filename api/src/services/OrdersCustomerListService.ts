...
export class OrdersCustomerListService {
  ordersRepository;
  ...
  async execute(userId: string) {
    try {
      const orders = await this.ordersRepository.findUserOrders(userId); // orders do usuario
      const cartsRepository = new CartsRepository(); //
      return orders.map(async (order) => {
        const { cart, items } = await cartsRepository.findCartById(
          order.cart_id
        );
        if (cart) {
          if (items.length > 0) {
            ...
            const itemsWithProducts = items.map(async (item) => {
              const product = await productsSearchService.execute({
                id: item.product_id,
              });
